...
RAW_STAT_FILE = '/home1/gaoy/svg/magenta/svg_vae_data/data-anypair/tfrecord_stats-00000-of-00001'
RAW_DATA_FILES = '/home1/gaoy/svg/magenta/svg_vae_data/data-anypair/tfrecord_data*'
URL_SPLITS = '/home1/gaoy/svg/magenta/svg_vae_data/data-single-glyph/svgvae_need_format/glyphazzn_urls.txt'
...
class GlyphAzznProblem(problem.Problem):
        ...
        yield {
            ...
            'targets_rel': np.array(
                example.features.feature['sequence'].float_list.value).astype(
                    np.float32).tolist(),
            'source_rel': np.array(
                example.features.feature['source_sequence'].float_list.value).astype(
                    np.float32).tolist(),
            'targets_rnd': np.array(
                example.features.feature['rendered'].float_list.value).astype(
                    np.float32).tolist(),
            'source_rnd': np.array(
                example.features.feature['source_rendered'].float_list.value).astype(
                    np.float32).tolist()
        }

  def example_reading_spec(self):
    data_fields = {'targets_rel': tf.FixedLenFeature([51*10], tf.float32),
                   'source_rel': tf.FixedLenFeature([51*10], tf.float32),
                   'targets_rnd': tf.FixedLenFeature([64*64], tf.float32),
                   'source_rnd': tf.FixedLenFeature([64*64], tf.float32),
  ...
  def preprocess_example(self, example, unused_mode, hparams):
    ...
    example['source_rel'] = tf.reshape(example['source_rel'], [51, 1, 10])
    # normalize (via gaussian)
    example['targets_rel'] = (example['targets_rel'] - self.mean_npz) / self.stdev_npz
    example['source_rel'] = (example['source_rel'] - self.mean_npz) / self.stdev_npz
    # redefine shape inside model!
    example['targets_psr'] = tf.reshape(example['targets_rnd'],
                                        [1, 64 * 64]) / 255.
    example['source_psr'] = tf.reshape(example['source_rnd'],
                                        [1, 64 * 64]) / 255.
    ...
    if hparams.just_render:
      ...
      example['source'] = tf.identity(example['source_rel'])
    return example
  ...
  def hparams(self, defaults, model_hparams):
    ...
    p.modality = {'inputs': modalities.ModalityType.IDENTITY,
                  ...
                  #'source' : modalities.ModalityType.IDENTITY}
