...
flags.DEFINE_string(
    'raw_data_file', '/home1/gaoy/svg/magenta/svg_vae_data/glyphs-parquetio-00000-of-00001',
    'File where the raw data is (in parquetio format).')
flags.DEFINE_string(
    'final_data_file', '/home1/gaoy/svg/magenta/svg_vae_data/tfrecord_dataset',
    'File where the final data will be saved (in tfrecord format).')
flags.DEFINE_string(
    'final_stats_file', '/home1/gaoy/svg/magenta/svg_vae_data/tfrecord_stats',
...
def _convert_to_path(g):
    ...
    # print('testing......')
    if int(g['binary_fp']) % 100 == 0:
        print(g['id'], g['binary_fp'])
    # print('testing......')
    # input()
...
def _decode_tfexample(serialized_example):
    ...
    return {
        ...
        'rendered': np.reshape(eg.features.feature['rendered'].float_list.value,
                               [64*64]).astype(np.float32).tolist(),
        'source_rendered': np.reshape(eg.features.feature['source_rendered'].float_list.value,
                                      [64*64]).astype(np.float32).tolist(),
    }
