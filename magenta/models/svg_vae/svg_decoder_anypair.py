...
class SVGDecoder(t2t_model.T2TModel):
    ...
    def pretrained_visual_encoder(self, features, hparams):
        ...
        with tf.variable_scope(tf.VariableScope(tf.AUTO_REUSE, ''),
            ...
            # source image feat
            vae_features_ = copy.copy(features)
            vae_features_['inputs'] = tf.reshape(vae_features_['source_psr'][:, -1, :],
                                                 [-1, 64, 64, 1])
            vae_features_['targets'] = vae_features_['inputs']
            # we want vae to return bottleneck
            vae_features_['bottleneck'] = tf.zeros((0, 128))
            sampled_bottleneck_source, _ = vae(vae_features_)
            ...
            # print(vae_features['targets_psr'])
            # input()
        ...
        return sampled_bottleneck - sampled_bottleneck_source
