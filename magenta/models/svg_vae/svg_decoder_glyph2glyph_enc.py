...
class SVGDecoder(t2t_model.T2TModel):
    ...
    def vis_encoder(self, sources_psr, targets_psr, targets_cls):
        ...
        with tf.variable_scope(tf.VariableScope(tf.AUTO_REUSE, ''),
            ...
            ret = tf.layers.Conv2D(base_depth, 5, 1,
                                   padding='SAME', activation=None)(ret)
            ret = ops.conditional_instance_norm(ret, clss, num_categories)
            ret = tf.nn.relu(ret)

            ret = tf.layers.Conv2D(base_depth, 5, 2,
                                   padding='SAME', activation=None)(ret)
            ret = ops.conditional_instance_norm(ret, clss, num_categories)
            ret = tf.nn.relu(ret)

            ret = tf.layers.Conv2D(2 * base_depth, 5, 1,
                                   padding='SAME', activation=None)(ret)
            ret = ops.conditional_instance_norm(ret, clss, num_categories)
            ret = tf.nn.relu(ret)

            ret = tf.layers.Conv2D(2 * base_depth, 5, 2,
                                   padding='SAME', activation=None)(ret)
            ret = ops.conditional_instance_norm(ret, clss, num_categories)
            ret = tf.nn.relu(ret)

            # new conv layer, to bring shape down
            ret = tf.layers.Conv2D(2 * bottleneck_bits, 4, 2,
                                   padding='SAME', activation=None)(ret)
            ret = ops.conditional_instance_norm(ret, clss, num_categories)
            ret = tf.nn.relu(ret)
    ...
    def render2cmd_v3_internal(self, features, hparams, train):
        ...
        with tf.variable_scope('render2cmd_v3_internal'):
            ...
            print('run stacking...')
            print("sample bottleneck shape sample bottleneck shape sample bottleneck shape ", sampled_bottleneck.shape)
            print("sources shape sources shape sources shape sources shape sources shape", sources.shape)
            # input()
